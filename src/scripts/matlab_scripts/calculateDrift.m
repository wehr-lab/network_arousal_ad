function [] = calculateDrift(varargin)
...
for currCell = 1:size(cellsToPlotCorr{1}, 1)
    ...
    figure
    for i = 1:length(Sessions)
        plot(allMaxResponses{i}(:, 1), '-', 'Color', cmap(i,:), 'LineWidth', 1.5);
        hold on
        plot(allMaxResponses{i}(:, 2), '--', 'Color', cmap(i,:), 'LineWidth', 1.5);
        hold on
        plot(allMaxResponses{i}(:, 3), '-.', 'Color', cmap(i,:), 'LineWidth', 1.5);
    end
    hold off
    xlim([1, 18]);
    ylim([-1, 15]);
